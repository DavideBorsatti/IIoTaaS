...
def application_services(appId, serviceId = None):
    ...
    if appId not in appids.keys():
        appids[appId] = {}
        appids[appId]['servicelist']=[]


    if request.method == 'GET':
        if serviceId != None:
            return json.dumps(appids[appId]['servicelist'][serviceId])
        else:
        ...
        appids[appId]['servicelist'].append(data)
        ...
        del appids[appId]['servicelist'][serviceId]
        return "ok"

#Get all Services list
@app.route('/mec_service_mgmt/v1/services', methods=["GET"])
def get_services():
    global appidS

    services=[]
    for key in appids:
        services.extend(appids[key]['servicelist'])
    return json.dumps(services)

#Get Services info
@app.route('/mec_service_mgmt/v1/services/<serviceId>', methods=["GET"])
def get_service(serviceId = None):
    global appids

    if serviceId != None:    
        for key in appids:
            for service in appids[key]['servicelist']:
                if service['serInstanceId'] == serviceId:
                    return json.dumps(service)
        abort(404, description="No Service with ServiceId: {} found".format(serviceId))
    else:
        services=[]
        for key in appids:
            services.extend(appids[key]['servicelist'])
